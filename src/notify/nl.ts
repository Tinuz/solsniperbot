...
const PHRASES: [RegExp, string | ((...m: string[]) => string)][] = [
  ...
  [/(\d+) forward trades made (\S+ SOL)/, (_, n, s) => `${n} trades onder deze instellingen maakten ${s}`],
  [/(\d+) forward trades/, (_, n) => `${n} trades onder deze instellingen`],
  [/(\d+) recent trades made (\S+ SOL)/, (_, n, s) => `${n} recente trades maakten ${s}`],
  [/(\d+) recent trades/, (_, n) => `${n} recente trades`],
  [/forward total /, 'totaal onder deze instellingen '],
  [/collecting forward data: /, 'verzamelt data onder deze instellingen: '],
  [/collecting data: /, 'verzamelt data: '],
  [/(\d+) launches over ([\d.]+)h recorded under these settings \(need (\d+) over ([\d.]+)h\)/, (_, n, h, need, hNeed) => `${n} launches in ${h} u onder deze instellingen (nodig: ${need} in ${hNeed} u)`],
  ...
  [/\(need (\d+)\)/, (_, n) => `(nodig: ${n})`],
  [/\benough forward data\b/g, 'genoeg data onder deze instellingen'],
